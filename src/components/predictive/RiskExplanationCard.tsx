    ...
    <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 shadow-sm space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            {citizenQuestions.map((item) => {
              ...
              return (
                <div
                  key={item.num}
                  className="rounded-xl border border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-850/50 overflow-hidden"
                >
                  <button
                    onClick={() => toggleQuestion(item.num)}
                    className="w-full flex items-center justify-between p-3 text-left hover:bg-slate-100/60 dark:hover:bg-slate-800/60 transition-colors"
                  >
                    <div className="flex items-center gap-2.5 min-w-0 pr-2">
                      <span className="w-5 h-5 rounded-full bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 flex items-center justify-center text-[10px] font-bold shrink-0 font-mono">
                        {item.num}
                      </span>
                  ...
                  {isExpanded && (
                    <div className="p-3 pt-0 text-xs border-t border-slate-100 dark:border-slate-800/60 mt-1">
                      {item.isList && Array.isArray(item.a) ? (
                        <ul className="space-y-1 mt-1.5">
                          {item.a.map((line: string, i: number) => (
                            <li
                              key={i}
                              className="flex items-start gap-1.5 text-slate-700 dark:text-slate-300 leading-relaxed"
                            >
      ...
      {activeTab === 'TECHNICAL_EXPLANATION' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
          ...
          <div className="space-y-1.5 p-3 rounded-xl bg-rose-50/50 dark:bg-rose-950/20 border border-rose-200/50 dark:border-rose-900/30">
          ...
          <div className="space-y-1.5 p-3 rounded-xl bg-emerald-50/50 dark:bg-emerald-950/20 border border-emerald-200/50 dark:border-emerald-900/30">
          ...
          <div className="col-span-1 md:col-span-2 space-y-1.5 p-3 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800">
            <span className="font-semibold text-slate-800 dark:text-slate-200 flex items-center gap-1.5">
